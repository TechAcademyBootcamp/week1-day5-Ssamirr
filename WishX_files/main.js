...
//---------------------> Update Phone-prefixes due to selected country <---------------------//
    $('#user-country .select_body li').click(function (){
        let country_id = $(this).data('value');
        let from_page = $('.from-page').data('page');
        let url = $('.prefix-url').data('url') + '?country=' + country_id + "&page=" + from_page;
        $.ajax({
            url: url,
            type: 'GET',
            success: function (data) {
    ...
    if (getCookie('cookies_strategy') !== 'True'){
        $('#cookie').removeClass('hide');
    }

    if ($("section").is("#cookie")) {
